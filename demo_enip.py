        ...
        if count <= 100:
            try:
                ...
                if data_temp != b'':
                    ...
                    log.info("%s request:%s" % (" ", data_recv))
                    ...
                    try:
                        ...
                        log.info("%s response:%s" % (" ", response_str.decode('utf-8')))
                        count = 0
                    except:
                        # time_now = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
                        ...
                        # print("%s response:%s" % ("Lucky!", '0300001611d00005001000c0010ac1020100c2020200'))
                        log.warning("%s response:%s" % ("Lucky!", '0300001611d00005001000c0010ac1020100c2020200'))
                else:
                    count += 1
            ...
            # time_now = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
            # print("%s connection from %s:%s has been broken!" % (" ", addr[0], addr[1]))
...
if __name__ == "__main__":
    ...
    if ip_addr == '':
        ...
        try:
            ...
            log.error("error, You have to input your ip address")
    openEnip(ip_addr)
# print(find(data))
