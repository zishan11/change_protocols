...
def cr3link(sock,addr):
    ...
    log.info("Accept new connection from {0}:{1}".format(addr[0],addr[1]))
    count = 0
    id = 0
    while True:
        if count <= 100:
            try:
                ...
                if data_temp != b'':
                    ...
                    try:
                        ...
                        log.info("%s response:%s" % (" ", response_data))
                        count = 0
                    except:
                        # time_now = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
                        # print('%s response cannot find!%s'%(time_now,data_recv))
                        log.warning('%s response cannot find!%s'%(" ",data_recv))
                        sock.send(binascii.a2b_hex('0300001611d00005001000c0010ac1020100c2020200'))
                        log.warning("%s response:%s" % (" ", '0300001611d00005001000c0010ac1020100c2020200'))
