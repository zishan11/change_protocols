...
def tcplink(sock, addr):
    ...
    log.info("Accept new connection from %s:%s" %addr)
    # CNCLog(addr,"Accept new connection from %s:%s" %addr)
    # print(addr)
    # sock.send(b'Welcome!')
    count = 0
    ...
    while True:
        ...
        try:
            data_temp = sock.recv(1024)
            log.info("no split request:%s"%data_temp)
            # CNCLog(addr,data_temp)
            # print(data_temp)
            ...
            if len(request_list) >= 1:
                ...
                for i in request_list:
                    ...
                    if results != ():
                        ...
                        log.info("results: %s" % results[0]["response_data"])
                        # CNCLog(addr,)
                    elif i_function != '':
                        # CNCLog(addr,i_function)
                        # print(i_function)
                        #对于未知的包，统统抓取
                        ...
                        print(sql_clause)
                        log.warning("can't find the unique response in database,but can be classified by function")
                        results = connect_database.searchData(db,sql_clause)
                        log.warning("response:%s "%results[0]['response_data'])
                        sock.send(binascii.a2b_hex(results[0]['response_data']))
                        # print(results)
    ...
    log.info('Connection from %s:%s closed.' % addr)
    # CNCLog(addr,'Connection from %s:%s closed.' % addr)
...
def openFanuc(ip,port=8193):
    ...
    # CNCLog((0,0),'Waiting for connecting...')
    '''
    建立连接的server
    '''
